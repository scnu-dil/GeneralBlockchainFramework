...
class client_config:
    ...
    # --------------------------------------
    # configure below
    contract_info_file = "./contractsbin/contract.ini"  # 保存已部署合约信息的文件
    account_keyfile_path = "./contractsbin/accounts"  # 保存keystore文件的路径，在此路径下,keystore文件以 [name].keystore命名
    ...
    logdir = "./contractsbin/logs"  # 默认日志输出目录，该目录必须先建立
    ...
    channel_node_cert = "./contractsbin/sdk.crt"  # 采用channel协议时，需要设置sdk证书,如采用rpc协议通信，这里可以留空
    channel_node_key = "./contractsbin/sdk.key"   # 采用channel协议时，需要设置sdk私钥,如采用rpc协议通信，这里可以留空
