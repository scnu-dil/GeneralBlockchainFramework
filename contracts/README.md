...
> solc编译器下载成功后，解压，将其中的 solc.exe 文件复制 ${contracts}\bin 目录下。若 contracts 路为 D:\\open-source\\contracts, 则 solc.exe 文件复制路径为D:\\open-source\\contracts\\bin\\solc.exe
...
> 若没有执行以上初始化步骤，需要将`temp/`目录下的`sol`代码手动编译成`bin`和`abi`文件并放置于`temp`目录，才可以部署和调用相应合约。合约编译可以使用[remix](https://remix.ethereum.org)
