...
> solc编译器下载成功后，解压，将其中的 solc.exe 文件复制 ${sdk}\bin 目录下。若 sdk 路为 D:\\open-source\\sdk, 则 solc.exe 文件复制路径为D:\\open-source\\sdk\\bin\\solc.exe
...
若sdk证书和私钥路径分别为`bin/sdk.crt`和`bin/sdk.key`，则`client_config.py`中相关配置项如下:
```bash
