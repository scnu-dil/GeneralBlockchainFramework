...
install_python3()
{
	...
	python_versions=$(pyenv virtualenvs | grep "sdk")
	if [ -z "${python_versions}" ];then
	    execute_cmd "pyenv virtualenv ${version} sdk"
	fi
}
...
python_init()
{
  ...
  if [ ${ret} = "1" ];then
    ...
    if [ $? -eq 0 ];then
        ...
        if [ ! -z "${python_versions}" ];then
            LOG_INFO "already install python ${version}, please activate the version with command: pyenv activate sdk"
            return
        fi
    fi
    ...
    source ${shell_rc}
    install_python3
    LOG_INFO "install python ${version} success, please activate with command: pyenv activate sdk"
  else
